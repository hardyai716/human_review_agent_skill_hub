...
```json
{
  ...
  "permission_level": "readonly | writable | confirmation_required | high_risk",
  "source_tier": "semantic_layer | governed_dataset | curated_raw_sql | raw_exploration | scenario_reference_preflight",
  "scenario_key": "string",
  "metric_id": "string",
  "review_required": true,
  "fallback_reason": "string",
  "execution_mode": "mock_readonly_no_real_query | readonly_no_real_query | real_readonly_query",
  ...
  "status": "success | failed | timeout | degraded | blocked",
  "latency_ms": 0,
  "fallback_action": "string"
}
```

阶段 1 P1 已先接入 mock / 只读预检记录。后续接入真实只读 Tool 后，`ToolCallRecord` 应同时支持真实只读执行记录；mock 模式下 `real_query_executed=false`，真实只读模式下必须写明 source tier、查询范围、指标口径、过滤条件、质量检查和 source_footer。通知、写状态和高风险动作仍必须走人工确认。
