...
def build_query_plan(sample: dict[str, Any]) -> dict[str, Any]:
    ...
    query_plan = {
        ...
        "required_hygiene_filters": [
            {
                "group": "A_project_title_blacklist",
                "field": "project_title",
                "operator": "NOT LIKE",
                "values": [
                    "%虚假%",
            ...
            {
                "group": "C_reason_exclusion",
                "field": "reason",
                "operator": "NOT IN",
                "values": ["recall_skip_L6", "fatal_output"],
            },
            {
                "group": "D_mach_root_label_allowlist_with_null",
                "field": "mach_root_label_name",
                "operator": "IS NULL OR IN",
                "values": [
                    "不良行为或争议价值观",
                    "侵犯未成年权益",
                    "偏激社会情绪和涉外言论",
