...
def scenario_bundle_skill_md(scenario_key: str, bundle_name: str) -> str:
    return f"""---
name: {bundle_name}
description: "当用户需要处理 {scenario_key} 打标率场景时使用：生成只读全等级分级、汇总统计、两周期剔除+1同意对比、报表、飞书 Card JSON、显式私聊推送、POC 路由和本地跟踪；默认 debug_only，真实发送和在线导入必须用户确认。"
allowed-tools:
  - Read
  - Bash
---
...
def build_scenario_bundle(scenario_key: str, dry_run: bool) -> None:
    ...
    for source_rel, target_rel in SCENARIO_BUNDLE_SCRIPT_SOURCES:
        ...
        if source_rel.endswith("sheet_importer.py"):
            ...
            side_effects = (
                "local_files_only; optional_online_sheet_write_explicit_opt_in; "
                "optional_interactive_card_dm_send_explicit_opt_in"
            )
        copy_or_transform_file(
            source=source,
            target=target,
